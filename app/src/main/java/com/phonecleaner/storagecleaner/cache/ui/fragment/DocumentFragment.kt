...
import com.phonecleaner.storagecleaner.cache.databinding.FragmentDocumentBinding
...
import com.phonecleaner.storagecleaner.cache.viewmodel.DocumentViewModel
...
class DocumentFragment : BaseFragment(), View.OnClickListener {
    ...
    override fun initUi() {
        ...
        binding.toolbar.tvTitle.text = getString(R.string.document)
        binding.toolbar.tvCreateFolder.isGone = true
    }

    override fun initData() {
        viewModel.getListFileDocument()
    }
    ...
    private fun initHeaderExplorerRecycler() {
        context?.let {
            ...
            headerExplorerAdapter.setData(arrayListOf(FileApp(name = getString(R.string.document))))
        }
    }
    ...
    private fun initObserve() {
        viewModel.listFileDocumentLiveData.observe(this) {
            when (it?.getStatus()) {
        ...
        with(mainViewModel) {
            sortedByLiveData.observe(viewLifecycleOwner) { condition ->
                when (condition) {
                    ...
                    Constants.SORTED_BY_TYPE_FROM_A_TO_Z -> {
                        try {
                            Timber.tag(myTag).d("sort by : $condition")
                            listFile.sortBy { fileApp ->
                                fileApp.type
                            }
                        } catch (e: Exception) {
    ...
    override fun onLayoutChange(type: LayoutType) {
        when (type) {
            LayoutType.LINEAR -> {
//                binding.toolbar.imgChangeView.setImageResource(R.drawable.ic_toolbar_change_view_linear)
                context?.let { ct ->
...
//                )
            ...
            LayoutType.GRID -> {
//                binding.toolbar.imgChangeView.setImageResource(R.drawable.ic_toolbar_change_view_grid)
                context?.let { ct ->
...
//            act.window.statusBarColor = ContextCompat.getColor(act, R.color.blue)
    ...
    companion object {
        fun onSetupView(): DocumentFragment {

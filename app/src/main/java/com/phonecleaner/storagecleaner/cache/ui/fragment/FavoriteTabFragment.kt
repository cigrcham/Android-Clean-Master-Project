...
@AndroidEntryPoint
class FavoriteTabFragment : BaseFragment(), View.OnClickListener {
    ...
    private fun initObserve() {
        with(mainViewModel) {
            observe(favoriteFileLiveData, ::getData)
        }
    }
...
//                    binding.imgFileNotFound.isVisible = it.isEmpty()
                }
            }
    ...
    override val onClickEvent: OnClickEvent = object : OnClickEvent {
        override fun onClickItem(file: FileApp, position: Int) {
            val dialog = AddFavoriteDialog.onSetUpView(file, false)
